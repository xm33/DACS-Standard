...
A bundle’s canonical form is the RFC 8785 JCS serialisation of the bundle with the presentation field omitted. The bundle hash is sha256(canonical_form), hex-encoded. The domain-separated signed_bytes (above) is what the presentation signature actually signs. Verifiers MUST recompute both the canonical form and the domain-separated payload when validating. **For the siwd kind specifically, the verifier MUST parse the SIWD `message`, extract the `Resource` line carrying the bundle binding, and confirm it equals the verifier's independently-recomputed hex `signed_bytes` for the received bundle; a missing or mismatched Resource line MUST cause rejection** (otherwise a captured SIWD `{message, signature, address}` whose wallet controls the same primary claim could be reattached to a different bundle — the SIWD signature alone proves the wallet signed *some* message, not that it committed to *this* bundle). If the SIWD message carries multiple `Resource` lines, at least one MUST equal the recomputed `signed_bytes`.
...
find_claim(bundle, cr):
  ...
  for each c in bundle.claims:
    ...
    //   eff_expiry := min(c.expiresAt ?? ∞, vr.validUntil ?? (eff_verifiedAt + recipe(c.ref.scheme).defaultMaxAgeSec * 1000))
    // Presenter-supplied c.issuedAt/c.expiresAt cannot extend this window (clamped, §6.3.2). Absent window → fail closed.
