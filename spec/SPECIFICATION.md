...
CF-4 applies identically to the listing logical address above, the `dacs1-revoked:{sellerPrimaryClaim}:{listingId}:v{listingVersion}` revocation marker, and the DACS-5 role-specific bundle addresses (§10.4.3). `native_address` is `sha256` over the CF-4-encoded `logical_address`.
