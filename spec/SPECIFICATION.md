...
find_claim(bundle, cr):
  ...
  for each c in bundle.claims:
    ...
    if cr.maxAge AND (c.issuedAt is null OR (now - c.issuedAt) > cr.maxAge * 1000): continue   // now, c.issuedAt in unix ms; cr.maxAge in seconds → ×1000. Absent issuedAt fails closed (stale).
