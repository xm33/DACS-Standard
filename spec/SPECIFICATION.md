...
- An **identity claim reference scheme** — a typed reference to an external identifier (`did:…`, `erc8004:…`, `lei:…`, `domain:…`, `platform:google:…`), optionally paired with a DACS-2 verification method.
...
- A **service listing schema** — a signed, anchored JSON document declaring the bundle requirement, offering, deliverable, pipeline, accepted rails, and terms. The listing is the canonical contract for any transaction against the seller.
...
| CD-1 | §8.5.1 | economically-equal decimals (`"1.50"`=`"1.5"`) → identical hashes/signatures | `conformance/vectors/` (CD-1) |
