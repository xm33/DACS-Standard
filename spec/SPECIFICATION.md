...
- **Bundle consumption.** Two-sided lookup; one-sided-bundle → aborted-by-self classification; **divergence is an `outcome`/`phaseSummary` *contradiction* per the single §10.4.3 definition — advisory-field skew (finalisedAt, one-sided ratingRefs, amendment order) is NOT divergence** and MUST be reconciled to the unified bundle (preferring the scored party's own copy), so a perturbed advisory field cannot force a spurious dispute; a genuine contradiction → the consumer applies the §10.4.3(d) per-party policy (buyer's bundle for buyer-reputation, seller's for seller-reputation), and the deriver excludes the jobId (guard ii) — both using the SAME definition. "disputed" is a **consumer-side verdict**, not an `AttestationBundle.outcome` value (the outcome enum has no `disputed` member) — the test asserts the observable §10.4.3(d) behaviour, not an enum label.
