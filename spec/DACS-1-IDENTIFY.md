...
| Tier | Schemes | `identityTier` (§6.3.2.1) |
| --- | --- | --- |
| 1 — authority-issued | `lei`, `finra-crd`, `sam-uei`, `fedramp`, `cmmc`, `naics` | institutional |
| 2 — DID / ERC-8004 with verifiable proof | `did`, `erc8004` (verified) | verified |
| 3 — platform identifier | verified `domain`, OAuth / platform accounts | verified |
| 4 — plain signing key | `key` (and any unverified or stale claim) | self-declared |

This single ranking governs both the `presentedBy` selection below and the §6.3.2.1 `identityTier` derivation.
...
Institutional precedence is strict: a bundle holding both a verified `lei:` and a verified `did:` derives `"institutional"`. The three values are exactly the `identityTier` column of the §6.3.2 **Claim tiers** table — {authority-issued} → institutional, {DID-with-proof, platform} → verified, {plain key / unverified or stale} → self-declared — so this derivation and the presentedBy-selection rule cannot disagree.
...
**The rule.** A DACS logical address is colon-delimited. **Every variable segment that can carry an internal `:` MUST be percent-encoded (uppercase hex, e.g. `:` → `%3A`) before assembly; the fixed structural segments MUST NOT be encoded.** Without encoding the colon-bearing segments the segment boundaries are undecidable and two implementations derive different native addresses.

CF-4 applies identically to every logical-address kind; the variable (encode) vs fixed (don't-encode) segments are:
