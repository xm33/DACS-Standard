...
- (2) `bundleRefs` MUST be serialised in **canonical order — ascending lexicographic by `AttestationRef.contentHash`** (the same tie-break discipline as SE-5; because `contentHash` is a sha256 digest the ordering is total, and two refs sharing a `contentHash` reference byte-identical content and collapse to one entry), so two derivers that computed identical metrics over the same set cannot disagree on `bundleRefs` byte-order;
