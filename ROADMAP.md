...
| `pay-x402` conformance vector | Candidate | `pay-x402` settles **on-chain** (gasless USDC on its settlement chain, chain-verifiable like the `evm` rail via `settlementTxHash`/`chainId`, §9.5.7); the x402 protocol ships in production and the rail is **not** operator-gated, so there is **no live-path barrier**. What it lacks for full v0.1 reference-backing is a **§14 conformance vector** (and confirmation the reference implementation exercises it). Add the vector. |
...
| §7.9 conformance-summary completeness (editorial) | Candidate; editorial | The DACS-2 §7.9 role→rule-range summary omits four rule families that carry MUSTs — **PSP-1..5** (ParserSpec semantics), **WN-1..6** (verification warnings), **GOV-2/3** (anchoring-phase disclosure), **PA-1..3** (progressive anchoring). A conformance implementer reading §7.9 as the checklist would miss them; decide the owning role per family and add rows (no rule-text change). Also tighten §7.5.1(a)'s retry-summary wording, which reads looser than the governing VP-R1/VP-R3. Surfaced by the DACS-2 readability review (2026-06-07); no normative change. |
